...
class PartController extends AbstractController
{
    ...
    public function submitQuiz(int $id, Request $request): Response
    {
        ...
        if (!$token || !$this->isCsrfTokenValid('quiz_submit' . $id, $token)) {
            $this->logger->error('Invalid or missing CSRF token', ['quiz_id' => $id, 'user_id' => $user->getId()]);
        ...
        if (!is_array($answers)) {
            $this->logger->error('Invalid answers format', ['quiz_id' => $id, 'user_id' => $user->getId()]);
        ...
        $attempt->setAnswers($result['feedback'] ?? []);
